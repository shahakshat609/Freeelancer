...
class BroseProjects extends Component {

    
    static propTypes = {
        handleSubmit4: PropTypes.func.isRequired , 
        projectsubmit:  PropTypes.func.isRequired,
        get1projects: PropTypes.func.isRequired//why is this piece of code???
    };
    ...
    componentWillMount(){
        console.log("in broseprojects");
        
        API.getOpenProjects2()
        .then(data=>{
    ...
    render(){
        const projectNode=this.state.result.map((project,index)=>{
         return(
                <tr key={index+1}>
                <td className="text-left">{index+11}</td>
...
<br/>
                ...
                <Link to="/browseprojects"><button type="button" className="btn btn-light"
...
export default withRouter(BroseProjects);