...
var assignedprojects=require('../models/assignproject.js');
...
router.get('/openprojects2',function(req,res)
{
 ...
 if(req.session.email)
 {
  ...
  proj.find({'email_address':{$ne:req.session.email}}).skip(10).limit(10)
  .exec(function(err,proj)
{
  if(err)
  {
...
router.get('/openprojects',function(req,res)
{
 ...
 if(req.session.email)
 {
  ...
  console.log('Retreive Open Projects request API called');
  proj.find({'email_address':{$ne:req.session.email}}).limit(10)
  .exec(function(err,proj)
{
  if(err)
  {
...
//assined projects API
router.get('/assignedprojects',function(req,res)
{
  console.log('Retreive Assigned Projects request API called');
 if(req.session.email)
 {
   console.log(req.session.email);
   assignprojects=new assignedprojects();
   //newproj.email_address=req.body.email_address;
   //console.log(newproj.email_address);

  
  assignedprojects.find({'freelancer':req.session.email})
  .exec(function(err,proj)
{
  if(err)
  {
